...
class KernelBuilder:
    ...
    def build_kernel(self, forest_height, n_nodes, batch_size, rounds):
        ...
        def emit_any_op(g, engine, op, reads, writes):
            offload_ops = {"^", "-", "&", ">>", "+", "<", "*"}
            op_name = op[0]
            # To balance load, only offload for certain groups.
            if engine == "valu" and op_name in offload_ops and g < 12:
                dest = op[1]
                a1 = op[2]
                a2 = op[3]
                for i in range(8):
                    emit_op("alu", (op_name, dest + i, a1 + i, a2 + i), 
                            reads=[a1 + i, a2 + i], writes=[dest + i])
            else:
