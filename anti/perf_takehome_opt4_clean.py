    ...
    def build_kernel(self, forest_height, n_nodes, batch_size, rounds):
        ...
        def emit_round_group(rnd, g):
            ...
            if depth == 0:
                ...
                emit_op("valu", ("-", gA, v_idx[g], v_three), reads=vr(v_idx[g]) + vr(v_three), writes=vr(gA))
                emit_op("valu", (">>", gB, gA, v_one), reads=vr(gA) + vr(v_one), writes=vr(gB))
                emit_op("valu", ("&", vg, gA, v_one), reads=vr(gA) + vr(v_one), writes=vr(vg))
                emit_op("valu", ("multiply_add", gA, vg, v_diff[2][0], v_leaf[2][0]), reads=vr(vg) + vr(v_diff[2][0]) + vr(v_leaf[2][0]), writes=vr(gA))
                emit_op("valu", ("multiply_add", vg, vg, v_diff2_slope, v_diff2_base), reads=vr(vg) + vr(v_diff2_slope) + vr(v_diff2_base), writes=vr(vg))
                emit_op("valu", ("multiply_add", gA, gB, vg, gA), reads=vr(gB) + vr(vg) + vr(gA), writes=vr(gA))
                ...
                emit_op("valu", ("-", t_b0, v_idx[g], v_seven), reads=vr(v_idx[g])+vr(v_seven), writes=vr(t_b0))
                emit_op("valu", ("&", t_b0, t_b0, v_one), reads=vr(t_b0)+vr(v_one), writes=vr(t_b0))
                emit_op("valu", ("-", t_b1, v_idx[g], v_seven), reads=vr(v_idx[g])+vr(v_seven), writes=vr(t_b1))
                emit_op("valu", (">>", t_b1, t_b1, v_one), reads=vr(t_b1)+vr(v_one), writes=vr(t_b1))
                emit_op("valu", ("&", t_b1, t_b1, v_one), reads=vr(t_b1)+vr(v_one), writes=vr(t_b1))
                emit_op("valu", ("-", t_b2, v_idx[g], v_seven), reads=vr(v_idx[g])+vr(v_seven), writes=vr(t_b2))
                emit_op("valu", (">>", t_b2, t_b2, v_two), reads=vr(t_b2)+vr(v_two), writes=vr(t_b2))
                emit_op("valu", ("&", t_b2, t_b2, v_one), reads=vr(t_b2)+vr(v_one), writes=vr(t_b2))
                
                emit_op("valu", ("multiply_add", t_M01, t_b0, v_diff[3][0], v_leaf[3][0]), reads=vr(t_b0)+vr(v_diff[3][0])+vr(v_leaf[3][0]), writes=vr(t_M01))
                emit_op("valu", ("multiply_add", t_M23, t_b0, v_diff[3][1], v_leaf[3][1]), reads=vr(t_b0)+vr(v_diff[3][1])+vr(v_leaf[3][1]), writes=vr(t_M23))
                
                emit_op("valu", ("-", t_diff, t_M23, t_M01), reads=vr(t_M23)+vr(t_M01), writes=vr(t_diff))
                emit_op("valu", ("multiply_add", t_M0123, t_b1, t_diff, t_M01), reads=vr(t_b1)+vr(t_diff)+vr(t_M01), writes=vr(t_M0123))
                
                t_M45 = t_M01
                t_M67 = t_M23
                emit_op("valu", ("multiply_add", t_M45, t_b0, v_diff[3][2], v_leaf[3][2]), reads=vr(t_b0)+vr(v_diff[3][2])+vr(v_leaf[3][2]), writes=vr(t_M45))
                emit_op("valu", ("multiply_add", t_M67, t_b0, v_diff[3][3], v_leaf[3][3]), reads=vr(t_b0)+vr(v_diff[3][3])+vr(v_leaf[3][3]), writes=vr(t_M67))
                
                t_M4567 = t_M67
                emit_op("valu", ("-", t_diff, t_M67, t_M45), reads=vr(t_M67)+vr(t_M45), writes=vr(t_diff))
                emit_op("valu", ("multiply_add", t_M4567, t_b1, t_diff, t_M45), reads=vr(t_b1)+vr(t_diff)+vr(t_M45), writes=vr(t_M4567))
                
                t_val_final = t_M01
                emit_op("valu", ("-", t_diff, t_M4567, t_M0123), reads=vr(t_M4567)+vr(t_M0123), writes=vr(t_diff))
                emit_op("valu", ("multiply_add", t_val_final, t_b2, t_diff, t_M0123), reads=vr(t_b2)+vr(t_diff)+vr(t_M0123), writes=vr(t_val_final))
                
                emit_op("valu", ("^", v_val[g], v_val[g], t_val_final), reads=vr(v_val[g])+vr(t_val_final), writes=vr(v_val[g]))
