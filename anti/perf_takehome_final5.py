...
class KernelBuilder:
    ...
    def build_kernel(self, forest_height, n_nodes, batch_size, rounds):
        ...
        NG_TMP = 8
        v_tmp = [self.alloc_scratch(f"v_tmp_{g}", 8) for g in range(NG_TMP)]
        NG_A = 8
        NG_B = 8
        v_glob_A = [self.alloc_scratch(f"vgA_{ti}", 8) for ti in range(NG_A)]
        v_glob_B = [self.alloc_scratch(f"vgB_{ti}", 8) for ti in range(NG_B)]
        v_glob_C = [self.alloc_scratch(f"vgC_{ti}", 8) for ti in range(NG_TMP)]
        v_glob_D = [self.alloc_scratch(f"vgD_{ti}", 8) for ti in range(NG_TMP)]
        v_glob_E = [self.alloc_scratch(f"vgE_{ti}", 8) for ti in range(NG_TMP)]
        v_glob_F = [self.alloc_scratch(f"vgF_{ti}", 8) for ti in range(NG_TMP)]
            ...
            if g < NUM_OFFLOAD: 
                if default_eng == "valu":
                    if slot[0] == "multiply_add":
                        ...
                        for i in range(8):
                            t = tmp_madd[g][i]
                            ops.append({"engine": "alu", "slot": ("*", t, a+i, b+i), "reads": [a+i, b+i], "writes": [t], "g": g})
                            ops.append({"engine": "alu", "slot": ("+", dest+i, t, c_op+i), "reads": [t, c_op+i], "writes": [dest+i], "g": g})
                    elif slot[0] == "vbroadcast":
                        _, dest, src = slot
                        for i in range(8):
                            ops.append({"engine": "alu", "slot": ("+", dest+i, src, s_zero), "reads": [src, s_zero], "writes": [dest+i], "g": g})
                    else:
                        op, dest, a, b = slot
                        for i in range(8):
                            ops.append({"engine": "alu", "slot": (op, dest+i, a+i, b+i), "reads": [a+i, b+i], "writes": [dest+i], "g": g})
                elif default_eng == "load" and slot[0] == "load_offset":
                    _, dest, addr, off = slot
                    ops.append({"engine": "load", "slot": ("load", dest+off, addr+off), "reads": [addr+off], "writes": [dest+off], "g": g})
                else:
                    ops.append({"engine": default_eng, "slot": slot, "reads": reads, "writes": writes, "g": g})
            else:
                ops.append({"engine": default_eng, "slot": slot, "reads": reads, "writes": writes, "g": g})
        ...
        for g in range(32):
            current_group = g
            emit_any_op(g, "load", ("vload", v_idx[g], s_addr_idx[g]),
                    reads=[s_addr_idx[g]], writes=vr(v_idx[g]))
            emit_any_op(g, "load", ("vload", v_val[g], s_addr_val[g]),
        ...
        def emit_round_group(rnd, g):
            ...
            if depth == 0:
                ...
                emit_any_op(g, "valu", ("-", gA, v_idx[g], v_three),
                        reads=vr(v_idx[g]) + vr(v_three), writes=vr(gA))
                emit_any_op(g, "valu", (">>", gB, gA, v_one),
                        reads=vr(gA) + vr(v_one), writes=vr(gB))
                emit_any_op(g, "valu", ("&", vg, gA, v_one),
                        reads=vr(gA) + vr(v_one), writes=vr(vg))
                emit_any_op(g, "valu", ("multiply_add", gA, vg, v_diff[2][0], v_leaf[2][0]),
                        reads=vr(vg) + vr(v_diff[2][0]) + vr(v_leaf[2][0]),
                        writes=vr(gA))
                emit_any_op(g, "valu", ("multiply_add", vg, vg, v_diff2_slope, v_diff2_base),
                        reads=vr(vg) + vr(v_diff2_slope) + vr(v_diff2_base),
                        writes=vr(vg))
                emit_any_op(g, "valu", ("multiply_add", gA, gB, vg, gA),
                        reads=vr(gB) + vr(vg) + vr(gA), writes=vr(gA))
                emit_any_op(g, "valu", ("^", v_val[g], v_val[g], gA),
                        reads=vr(v_val[g]) + vr(gA), writes=vr(v_val[g]))
