    ...
    def build_kernel(self, forest_height, n_nodes, batch_size, rounds):
        ...
        for hi in range(len(HASH_STAGES)):
            if hi in (0, 2, 4):
                ...
                s_hash_val1[hi] = self.alloc_scratch(f"s_hv1_{hi}")
        ...
        # Vector constants (0, 2, 4): need load to temp + vbroadcast
        # Scalar constants (1, 3, 5): load directly
        s_hc_tmp0 = self.alloc_scratch("s_hc_tmp0")
        s_hc_tmp2 = self.alloc_scratch("s_hc_tmp2")
        s_hc_tmp4 = self.alloc_scratch("s_hc_tmp4")

        asm.add("load", ("const", s_hash_val1[1], HASH_STAGES[1][1]))
        asm.add("load", ("const", s_hash_val3[1], HASH_STAGES[1][4]))
        asm.emit()

        asm.add("load", ("const", s_hash_val1[3], HASH_STAGES[3][1]))
        asm.add("load", ("const", s_hash_val3[3], HASH_STAGES[3][4]))
        asm.emit()

        asm.add("load", ("const", s_hash_val1[5], HASH_STAGES[5][1]))
        asm.add("load", ("const", s_hash_val3[5], HASH_STAGES[5][4]))
        asm.emit()

        asm.add("load", ("const", s_hc_tmp0, HASH_STAGES[0][1]))
        asm.add("load", ("const", s_hc_tmp2, HASH_STAGES[2][1]))
        asm.emit()

        asm.add("load", ("const", s_hc_tmp4, HASH_STAGES[4][1]))
        asm.add("valu", ("vbroadcast", v_hash_val1[0], s_hc_tmp0))
        asm.add("valu", ("vbroadcast", v_hash_val1[2], s_hc_tmp2))
        asm.emit()

        asm.add("valu", ("vbroadcast", v_hash_val1[4], s_hc_tmp4))
        asm.emit()
        asm.emit()
        ...
        def schedule_ops(ops):
            ...
            while ready:
                ...
                for op_idx in scheduled_now:
                    ...
                    if ops[op_idx].get('g') == 0 and ops[op_idx].get('rnd') == 4:

                        print(f"Group 0 Rnd 4: Op={ops[op_idx].get('slot')}, parents={[ops[p].get('slot') for p in parents[op_idx]]}, cycle={current_cycle}")
