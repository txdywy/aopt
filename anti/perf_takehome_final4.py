            ...
            if g < NUM_OFFLOAD: 
                if default_eng == "valu":
                    if slot[0] == "multiply_add":
                        ...
                        for i in range(8):
                            t = tmp_madd[g][i]
                            ops.append({"engine": "alu", "slot": ("*", t, a+i, b+i), "reads": [a+i, b+i], "writes": [t]})
                            ops.append({"engine": "alu", "slot": ("+", dest+i, t, c_op+i), "reads": [t, c_op+i], "writes": [dest+i]})
                    elif slot[0] == "vbroadcast":
                        _, dest, src = slot
                        for i in range(8):
                            ops.append({"engine": "alu", "slot": ("+", dest+i, src, s_zero), "reads": [src, s_zero], "writes": [dest+i]})
                    else:
                        op, dest, a, b = slot
                        for i in range(8):
                            ops.append({"engine": "alu", "slot": (op, dest+i, a+i, b+i), "reads": [a+i, b+i], "writes": [dest+i]})
                elif default_eng == "load" and slot[0] == "load_offset":
                    _, dest, addr, off = slot
                    ops.append({"engine": "load", "slot": ("load", dest+off, addr+off), "reads": [addr+off], "writes": [dest+off]})
                else:
                    ops.append({"engine": default_eng, "slot": slot, "reads": reads, "writes": writes})
            else:
                ops.append({"engine": default_eng, "slot": slot, "reads": reads, "writes": writes})
