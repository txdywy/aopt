        ...
        def schedule_ops(ops):
            ...
            while ready:
                ...
                for op_idx in scheduled_now:
                    ...
                    if ops[op_idx].get('g') == 0:

                        # Print when we schedule the first op of each round for group 0

                        if ops[op_idx].get('slot')[0] == '^' and ops[op_idx].get('engine') == 'valu' and ops[op_idx].get('depth') == 0:

                            print(f'Group 0 Round {ops[op_idx].get("rnd")} (depth 0) starts at Cycle {current_cycle}')
