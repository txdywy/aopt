...
class KernelBuilder:
    ...
    def build_kernel(self, forest_height, n_nodes, batch_size, rounds):
        ...
        for hi in range(len(HASH_STAGES)):
            v_hash_val1[hi] = self.alloc_scratch(f"v_hv1_{hi}", 8)
            if hi in (1, 3, 5):
        ...
        # All 9 must be loaded as scalars and then vbroadcasted to vector registers
        s_hc_tmp = [self.alloc_scratch(f"s_hc_tmp_{i}") for i in range(9)]

        asm.add("load", ("const", s_hc_tmp[0], HASH_STAGES[0][1]))
        asm.add("load", ("const", s_hc_tmp[1], HASH_STAGES[1][1]))
        asm.emit()

        asm.add("load", ("const", s_hc_tmp[2], HASH_STAGES[1][4]))
        asm.add("load", ("const", s_hc_tmp[3], HASH_STAGES[2][1]))
        asm.emit()

        asm.add("load", ("const", s_hc_tmp[4], HASH_STAGES[3][1]))
        asm.add("load", ("const", s_hc_tmp[5], HASH_STAGES[3][4]))
        asm.add("valu", ("vbroadcast", v_hash_val1[0], s_hc_tmp[0]))
        asm.add("valu", ("vbroadcast", v_hash_val1[1], s_hc_tmp[1]))
        asm.emit()

        asm.add("load", ("const", s_hc_tmp[6], HASH_STAGES[4][1]))
        asm.add("load", ("const", s_hc_tmp[7], HASH_STAGES[5][1]))
        asm.add("valu", ("vbroadcast", v_hash_val3[1], s_hc_tmp[2]))
        asm.add("valu", ("vbroadcast", v_hash_val1[2], s_hc_tmp[3]))
        asm.emit()

        asm.add("load", ("const", s_hc_tmp[8], HASH_STAGES[5][4]))
        asm.add("valu", ("vbroadcast", v_hash_val1[3], s_hc_tmp[4]))
        asm.add("valu", ("vbroadcast", v_hash_val3[3], s_hc_tmp[5]))
        asm.emit()

        asm.add("valu", ("vbroadcast", v_hash_val1[4], s_hc_tmp[6]))
        asm.add("valu", ("vbroadcast", v_hash_val1[5], s_hc_tmp[7]))
        asm.emit()

        asm.add("valu", ("vbroadcast", v_hash_val3[5], s_hc_tmp[8]))
        asm.emit()
        ...
        def emit_round_group(rnd, g):
            ...
            if depth == 0:
                ...
                gC = v_glob_C[g % NG_C]
                gD = v_glob_D[g % NG_D]
                emit_any_op(g, "valu", ("-", vg, v_J[g], v_eight), reads=vr(v_J[g])+vr(v_eight), writes=vr(vg))
                ...
                emit_any_op(g, "valu", ("&", gA, gA, v_one), reads=vr(gA)+vr(v_one), writes=vr(gA))
                emit_any_op(g, "valu", ("-", gB, v_J[g], v_eight), reads=vr(v_J[g])+vr(v_eight), writes=vr(gB))
                emit_any_op(g, "valu", (">>", gB, gB, v_two), reads=vr(gB)+vr(v_two), writes=vr(gB))
                emit_any_op(g, "valu", ("&", gB, gB, v_one), reads=vr(gB)+vr(v_one), writes=vr(gB))

                emit_any_op(g, "valu", ("multiply_add", gC, vg, v_diff[3][0], v_leaf[3][0]), reads=vr(vg)+vr(v_diff[3][0])+vr(v_leaf[3][0]), writes=vr(gC))
                emit_any_op(g, "valu", ("multiply_add", gD, vg, v_diff[3][1], v_leaf[3][1]), reads=vr(vg)+vr(v_diff[3][1])+vr(v_leaf[3][1]), writes=vr(gD))
                ...
                emit_any_op(g, "valu", ("multiply_add", gD, vg, v_diff[3][2], v_leaf[3][2]), reads=vr(vg)+vr(v_diff[3][2])+vr(v_leaf[3][2]), writes=vr(gD))
                emit_any_op(g, "valu", ("multiply_add", vg, vg, v_diff[3][3], v_leaf[3][3]), reads=vr(vg)+vr(v_diff[3][3])+vr(v_leaf[3][3]), writes=vr(vg))
                ...
                emit_any_op(g, "valu", ("^", v_val[g], v_val[g], gC), reads=vr(v_val[g])+vr(gC), writes=vr(v_val[g]))
        ...
        self.v_J = v_J
        self.v_val = v_val
        self.s_addr_idx = s_addr_idx
        self.s_addr_val = s_addr_val
        self.s_vars = s_vars
        self.v_leaf = v_leaf
        self.v_hash_val1 = v_hash_val1
