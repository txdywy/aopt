    ...
    def build_kernel(self, forest_height, n_nodes, batch_size, rounds):
        ...
        def emit_round_group(rnd, g):
            ...
            if depth == 0:
                ...
                emit_op("valu", ("&", vg, gA, v_one),
                        reads=vr(gA) + vr(v_one), writes=vr(vg))
                emit_op("valu", ("multiply_add", gA, vg, v_diff[2][0], v_leaf[2][0]),
                        reads=vr(vg) + vr(v_diff[2][0]) + vr(v_leaf[2][0]),
                        writes=vr(gA))
                emit_op("valu", ("multiply_add", vg, vg, v_diff2_slope, v_diff2_base),
                        reads=vr(vg) + vr(v_diff2_slope) + vr(v_diff2_base),
                        writes=vr(vg))
                emit_op("valu", ("multiply_add", gA, gB, vg, gA),
        ...
        def schedule_ops(ops):
            ...
            # Artificial staggering: Make the first operation of group g depend on some operation
            # of group g-1, to force a staggered start and prevent engine clumping.
            first_op_of_group = {}
            for i, op in enumerate(ops):
                g = op.get('g')
                if g is not None and g not in first_op_of_group:
                    first_op_of_group[g] = i
            
            for g in range(4, 32):
                if g in first_op_of_group and g-4 in first_op_of_group:
                    # Make group g start after group g-4's first op
                    parents[first_op_of_group[g]].add(first_op_of_group[g-4])
